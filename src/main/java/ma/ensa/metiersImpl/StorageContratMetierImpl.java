...
@Service
@Transactional
public class StorageContratMetierImpl implements StorageContratMertier{


	  Logger log = LoggerFactory.getLogger(this.getClass().getName());
	  private final Path rootLocation = Paths.get("app-rh-files");
	  private final Path rootLocation2 = Paths.get("app-rh-files/pdf-contrat");
	  @Autowired
	  ...
	  public String store(MultipartFile file, String id) {
	    try {
			...
			Optional<Contrat> ats = this.contratDao.findById(Long.parseLong(id));
	    	if(ats.isPresent()) {
